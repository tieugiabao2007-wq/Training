...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Causal setup meta-label exploration on exact Exness M5")
...
def _load_dataset(args: argparse.Namespace) -> dict[str, Any]:
    ...
    features = build_meta_features(decisions, point)
    ...
    if len(x) != MAX_DEVELOPMENT_ROWS or (source_positions < 0).any():
        raise ValueError("Setup meta cycle requires 45,000 exact complete development rows")
    outcomes = build_horizon_outcomes(
        label_bars, x.index, directions, point=point
    )
    return {
        "bars": label_bars,
        "metadata": metadata,
...
def _select_inner(
    ...
    *,
    seed: int,
    inner_splits: int = 2,
    inner_max_train_rows: int = 24_000,
) -> tuple[dict[str, Any], dict[str, Any]]:
    minimum = max(5_000, min(10_000, len(x) - 1_000))
    splits = list(
        _splitter(inner_splits, minimum, inner_max_train_rows).split(
            x.index, outcomes[MAX_HORIZON]["label_end_time"]
        )
    )
...
def main() -> int:
    ...
    if output.exists():
        raise FileExistsError(f"Refusing to repeat existing setup-meta artifact: {output}")
    ...
    outer_splits = list(
        _splitter(3, 12_000, 30_000).split(
            x.index,
            data["outcomes"][next(iter(SETUP_SPECS))][MAX_HORIZON]["label_end_time"],
        )
    )
    if args.preflight:
        print(
            json.dumps(
                {
                    ...
                    "cost_model": "side_aware_exact_bar_spread_plus_0.50bps_buffer",
    ...
    for fold, split in enumerate(outer_splits, start=1):
        ...
        for offset, setup_id in enumerate(SETUP_SPECS):
            print(f"[setup-meta] fold={fold}/3 setup={setup_id} phase=inner", flush=True)
            outer_train = split.train_positions
            x_train = x.iloc[outer_train]
            source_train = data["source_positions"][outer_train]
            outcomes_train = {
                horizon: data["outcomes"][setup_id][horizon].iloc[outer_train]
                for horizon in HORIZONS
            }
            frozen, inner_audit = _select_inner(
                ...
                outcomes_train,
                source_train,
                seed=17_000 + fold * 100 + offset * 10,
            )
            print(
                f"[setup-meta] fold={fold}/3 setup={setup_id} "
            ...
            candidate, control, fit_audit = _outer_pair(
                ...
                split.test_positions,
                fold=fold,
                seed=18_000 + fold * 100 + offset * 10,
            )
    ...
    for setup_id, spec in SETUP_SPECS.items():
        ...
        candidate = pd.concat(candidate_nonempty).sort_index() if candidate_nonempty else pd.DataFrame(
            columns=["direction", "actual_direction", "net_return", "horizon_bars", "fold"]
        )
        control = pd.concat(control_nonempty).sort_index() if control_nonempty else pd.DataFrame(
            columns=["direction", "actual_direction", "net_return", "horizon_bars", "fold"]
        )
        ...
        checks = survival_checks(candidate_metrics, control_metrics)
    ...
    decision = "ESCALATE_BEST_SETUP_META_SURVIVOR" if selected else "REJECT_ALL_SETUP_META_CANDIDATES"
    summary = {
        ...
        "protocol": {
            ...
            "objective_order": ["outer_oos_after_dynamic_cost_profit", "drawdown_and_stability", "accuracy_wilson"],
        ...
        "source_provenance": {
            ...
            "core": {"path": str((PROJECT_ROOT / 'src/gold_ai/setup_meta_label.py').resolve()), "sha256": sha256_file(PROJECT_ROOT / 'src/gold_ai/setup_meta_label.py')},
        },
